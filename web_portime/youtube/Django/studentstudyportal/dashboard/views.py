...
def notes(request):
    if request.method=="POST":
        ...
        form=NotesForm()
    notesEl=Notes.objects.filter(user=request.user)
...
def homework(request):
    if request.method== "POST":
        ...
        form = HomeworkForm()
    homeworks = Homewwork.objects.filter(user=request.user)
    if len(homeworks) == 0:
        homework_done = True
    else:
        homework_done = False
...
def update_homework(request,pk=None):
    homework=Homewwork.objects.get(id=pk)
...
@login_required
def delete_hoemwork(request,pk=None):
    Homewwork.objects.get(id=pk).delete()
    return redirect("homework")
            ...
            if 'input' in request.POST:
                ...
                if input and int(input) > 0:
                    ...
                    if first == 'kilogram' and second == 'pound':
                        answer = f'{input} foot = {int(input) * 2.20462} pound'
...
@login_required
def profile(request):
    homework=Homewwork.objects.filter(is_finished=False,user=request.user)
